...
async function register(event){event.preventDefault();const btn=document.getElementById('registerSubmit'),msg=document.getElementById('registerMessage');btn.disabled=true;msg.textContent=pendingAvatarData?'アイコンをアップロードしています…':'登録しています…';try{const removeAvatar=document.getElementById('avatarClearButton').dataset.remove==='true';const candidate={userId:profile?.userId||uid(),nickname:document.getElementById('nicknameInput').value.trim(),faculty:document.getElementById('facultyInput').value.trim(),department:document.getElementById('departmentInput').value.trim(),teacherEmail:document.getElementById('teacherEmailInput').value.trim(),avatarUrl:removeAvatar?'':(profile?.avatarUrl||''),avatarLocal:removeAvatar?'':(pendingAvatarData||profile?.avatarLocal||'')};const result=await apiPost({action:'register',...candidate,avatarData:pendingAvatarData||'',removeAvatar:removeAvatar?'true':'false'});candidate.avatarUrl=result.avatarUrl||candidate.avatarUrl||'';profile=candidate;pendingAvatarData='';document.getElementById('avatarClearButton').dataset.remove='false';saveProfile();updateProfileUI();document.getElementById('registerDialog').close();toast(result.avatarWarning?'登録完了（アイコンはこの端末内に保存しました）':'参加者登録が完了しました');loadRanking();}catch(e){msg.textContent='登録できませんでした：'+e.message;}finally{btn.disabled=false;}}
function openProfile(){if(profile){document.getElementById('nicknameInput').value=profile.nickname||'';document.getElementById('facultyInput').value=profile.faculty||'';document.getElementById('departmentInput').value=profile.department||'';document.getElementById('teacherEmailInput').value=profile.teacherEmail||'';}pendingAvatarData='';document.getElementById('avatarClearButton').dataset.remove='false';updateAvatarPreview(profile?.avatarLocal||profile?.avatarUrl||'',profile?.nickname||'');document.getElementById('avatarInput').value='';document.getElementById('registerMessage').textContent='';document.getElementById('registerDialog').showModal();}
function updateProfileUI(){const ownAvatar=profile?.avatarLocal||profile?.avatarUrl||'';applyAvatar(document.getElementById('profileBtn'),ownAvatar,profile?.nickname);applyAvatar(document.querySelector('.avatar.big'),ownAvatar,profile?.nickname);}
