...
function applyCloudData(cloud){
  ...
  state.serverWeeklyMinutes=Number(cloud.weeklyMinutes)||0;state.serverTotalMinutes=Number(cloud.totalMinutes)||0;state.cheerPoints=Number(cloud.cheerPoints)||0;state.medalPoints=Number(cloud.medalPoints)||0;state.streakPoints=Number(cloud.streakPoints)||0;state.missionPoints=Number(cloud.missionPoints)||0;if(typeof window.applyMissionCloudRecords==='function'&&Array.isArray(cloud.missionRecords))window.applyMissionCloudRecords(cloud.missionRecords);state.currentStreak=Number(cloud.currentStreak)||0;state.todayServerMinutes=Number(cloud.todayMinutes)||0;state.badgeHistory=Array.isArray(cloud.badgeHistory)?cloud.badgeHistory:[];state.lastCloudSync=cloud.updatedAt||'';
