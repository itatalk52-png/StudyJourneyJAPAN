...
function renderRanking(rows){const list=document.getElementById('friendsList');list.innerHTML='';if(!rows.length){list.innerHTML='<div class="empty-ranking">まだランキング記録がありません。<br>タイマーを合計1分以上動かして停止すると反映されます。</div>';return;}rows.forEach(row=>{const mine=profile&&row.userId===profile.userId;if(mine){
  ...
  state.todayServerMinutes=Number(row.todayMinutes)||0;
  const myTodayRankEl=document.getElementById('myTodayRank');
  if(myTodayRankEl)myTodayRankEl.textContent=`今日ランキング ${Number(row.todayRank)||'-'}位`;
...
}const div=document.createElement('div');div.className='friend-row'+(mine?' mine':'');const totalPoints=Number(row.totalPoints)||0;const action=mine?'':`<button type="button" class="cheer-btn ${row.dormant?'dormant':''}" ${row.cheeredToday?'disabled':''}>${row.cheeredToday?'送信済み':`エールを送る🎉 +${row.cheerValue}pt`}</button>`;div.innerHTML=`<span class="rank-no">${Number(row.rank)||'-'}</span>${avatarMarkup(row.avatarUrl,row.nickname)}<div><strong>${escapeHtml(row.nickname)}${mine?'（あなた）':''}</strong><small>${escapeHtml(row.currentPrefecture||'沖縄県')} ${row.faculty?'・'+escapeHtml(row.faculty):''}</small><div class="points-breakdown">学習 ${Number(row.totalMinutes)||0}pt・問題集 ${Number(row.missionPoints)||0}pt・メダル ${Number(row.medalPoints)||0}pt・連続 ${Number(row.streakPoints)||0}pt・エール ${formatPoints(row.cheerPoints)}pt</div></div><span class="friend-score"><span class="weekly-time-label">今週の簿記勉強時間</span><strong>${formatStudyDuration(row.weeklyMinutes)}</strong><small>${formatPoints(totalPoints)} pt</small><span class="today-study-label">今日の簿記勉強時間</span><strong class="today-study-time">${formatStudyDuration(row.todayMinutes)}</strong><small class="today-rank">今日ランキング ${Number(row.todayRank)||'-'}位</small></span>${action}`;const btn=div.querySelector('.cheer-btn');if(btn&&!btn.disabled)btn.onclick=()=>sendCheer(row.userId,row.nickname,btn);list.appendChild(div);});}
